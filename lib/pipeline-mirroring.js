...
import { queryVirtuoso } from './sparql-virtuoso';
...
export async function run(task) {
  try {
    ...
    await grph.getTriplesInFileAndApplyByBatch(task, async (store) => {
      const renamedTriples = await renameTriples(store);
...
async function renameUri(oldUri) {
  const queryResult = await queryVirtuoso(`
