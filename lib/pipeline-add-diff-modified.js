...
export async function run(task, signal = {}) {
  try {
    ...
    await getTriplesInFileAndApplyByBatch(task, async (store, derivedFrom) => {
      ...
      // Not the same as other pipelines. There is only going to be 1
      // derivedFrom. Use it that way to later be able to add to this file
      // directly.
      ...
      subjects.forEach((subject) => {
        ...
        store.addQuad(subject, modifiedBy, task.task);
      });
    ...
    await getDeletedTriplesInFileAndApplyByBatch(
      ...
      async (store, derivedFrom) => {
        ...
        subjects.forEach((subject) => {
          ...
          insertStore.addQuad(subject, modifiedBy, task.task);
        });
