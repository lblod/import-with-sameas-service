      ...
      INSERT DATA {
        GRAPH ${rst.termToString(graph)} {
          ...
          ${rst.termToString(logicalFile)}
            ...
            prov:wasDerivedFrom ${rst.termToString(derivedFrom)};
            dct:creator <http://lblod.data.gift/services/harvesting-import-service> ;
            dct:created ${rst.termToString(now)} ;
            dct:modified ${rst.termToString(now)} ;
            dct:format "text/turtle" ;
 ...
 * @async
 * @function
 * @param {String|Buffer|URL} path - Path to the file to remove.
 * @returns {Promise} Returned promise from the built-in Node Filesystem library.
 */
