...
import { storeToSparql, sleep } from './utils';
import { SPARQL_PREFIXES, BATCH_SIZE, SLEEP_TIME } from '../constants';
...
export async function getTriplesInGraph(task, defaultLimitSize = 200) {
  const graphResponse = await updateSudo(
    `
     ${SPARQL_PREFIXES}
  ...
  const countResponse = await querySudo(
    ...
    SELECT (count(?s) as ?count) WHERE {
      GRAPH ${termToString(graphData.graph)} {
        ?s ?p ?o .
      }
    }
  `,
  ...
  for (let page = 0; page < pagesCount; page++) {
    ...
    const queryResponse = await querySudo(
      `
...
/**
 * Reads the input files matched by the given query and applies `callback` to
 * each batch of triples (a batch is at most `sizeBuf` lines of a single file).
 *
 * Optional per-file lifecycle hooks, used to make callers resumable:
 *   - `fileHooks.shouldProcessFile(derivedFrom)` is awaited before a file is
 *     read; returning a falsy value skips the file entirely (no `callback`, no
 *     `onFileComplete`).
 *   - `fileHooks.onFileComplete(derivedFrom)` is awaited once, after a file has
 *     been fully read, for every file that was not skipped.
 *
 * The hooks are keyed on `derivedFrom`, which assumes one input file (path) per
 * `derivedFrom`. That holds for harvested files; finalizing/skipping per
 * `derivedFrom` would be unsafe if a single source mapped to multiple paths.
 */
export async function fetchFileInputContainerAndApplyBatch(
  ...
  const fileResponse = await querySudo(
    queryInputContainer,
    {},
    connectionOptions,
  );
  ...
  for (const f of files) {
    // Allow callers to skip files whose output was already produced by a
    // previous (interrupted) run, so a restart resumes instead of redoing work.
    if (
      fileHooks.shouldProcessFile &&
      !(await fileHooks.shouldProcessFile(f.derivedFrom))
    ) {
      continue;
    }
    ...
    if (buf.length || isEmpty) {
      const arrayStream = new ArrayReadableStream(buf);
      const store = await streamToN3Store(arrayStream);
      await callback(store, f.derivedFrom);
      buf = [];
    }
    // The whole input file has been read; let the caller finalize its output
    // so a completed file is durably recorded and skipped on a restart.
    if (fileHooks.onFileComplete) {
      await fileHooks.onFileComplete(f.derivedFrom);
    }
  }
}
/**
 * In the diff step, only the newly inserted file is added to the result graph container.
 * As a result, only "new-insert" files will be fetched in this function.
 * Diff step can produce multiple output files, but only one is considered the result (the new-inserts file)
 * This is why we extract the file from the `task:hasGraph/task:hasFile` relation.
 * This may need cleanup for clarity.
...
export async function writeTriplesToGraph(
  ...
  for (let page = 0; page < pages; page++) {
    ...
    try {
      await updateSudo(queryStr, {}, connectionOptions);
    } catch (e) {
      if (batchSize > 1) {
        console.warn(
    ...
    try {
      ...
      if (batchSize > 1) {
        ...
        if (skipFailures) {
          ...
          const tripleString = [
            termToString(triples[start].subject),
            termToString(triples[start].predicate),
            termToString(triples[start].object),
          ].join(' ');
          throw new Error(
...
async function deleteTriplesFromGraphWithoutBatching(graph, store) {
  ...
  const toRemoveString = await storeToSparql(store);
  return updateSudo(
    `
    DELETE DATA {
      GRAPH ${termToString(graph)} {
    ...
    INSERT DATA {
      GRAPH ${termToString(task.graph)} {
        ${termToString(container.node)}
          ...
          task:hasGraph ${termToString(graph)} .
        ${termToString(task.task)}
