...
import { updateVirtuoso } from './sparql-virtuoso';
...
export async function fetchFileInputContainerAndApplyBatch(queryInputContainer, callback) {
  const fileResponse = await mas.querySudo(queryInputContainer);
  const parser = new sjp.SparqlJsonParser();
...
export async function getTriplesInFileAndApplyByBatch(task, callback = async (_store) => { }) {
...
export async function getDeletedTriplesInFileAndApplyByBatch(task, callback = async (_store) => { }) {
