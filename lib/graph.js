...
import { sleep, parseResult, askQuery, filterAsync } from './utils';
import fs from 'fs';
...
export async function writeTriplesToGraph(graph, ttlTriples, batchSize = 100) {
  ...
  while (triples.length) {
    let batch = triples.splice(0, batchSize);
    batch = await filterAsync(batch, (triple) => askQuery(graph, triple), true)
    const queryStr = `
      INSERT DATA {
